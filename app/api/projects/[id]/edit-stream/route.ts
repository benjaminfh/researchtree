    ...
    return await withProjectLock(params.id, async () => {
      ...
      try {
        if (store.mode === 'pg') {
          ...
          registerStream(params.id, abortController, targetBranch);
          ...
          const stream = new ReadableStream<Uint8Array>({
            async start(controllerStream) {
                  ...
                  for await (const chunk of streamAssistantCompletion({
                    ...
                    if (chunk.type === 'thinking') {
                      ...
                      controllerStream.enqueue(
                        encodeChunk(`${JSON.stringify({ type: 'thinking', content: contentChunk, append: chunk.append })}\n`)
                      );
                      continue;
                    }
                    if (chunk.type === 'thinking_signature') {
                      streamBlocks.push({
                        ...
                        signature: contentChunk
                      });
                      controllerStream.enqueue(
                        encodeChunk(`${JSON.stringify({ type: 'thinking_signature', content: contentChunk, append: chunk.append })}\n`)
                      );
                      continue;
                    }
                    const lastText = streamBlocks[streamBlocks.length - 1];
                    if (lastText?.type === 'text') {
                      lastText.text += contentChunk;
                    } else {
                      streamBlocks.push({ type: 'text', text: contentChunk });
                    }
                    buffered += contentChunk;
                    yieldedText = true;
                    controllerStream.enqueue(encodeChunk(`${JSON.stringify({ type: 'text', content: contentChunk })}\n`));
                  }
                } catch (error) {
                  streamError = error;
                }
              ...
              if (!yieldedText && editedNode.role === 'user' && !abortController.signal.aborted && streamError == null) {
                streamError = new Error('LLM returned empty response');
              }

              try {
                if (editedNode.role === 'user' && buffered.trim().length > 0) {
                ...
                releaseStream(params.id, targetBranch);
                releaseRefLock();
              }
        ...
        const { createBranch, switchBranch } = await import('@git/branches');
