...
function buildSystemPrompt(): string {
  return [
    ...
    'Rules (with examples):',
    '1) canvas_grep query must be text or /regex/flags. Example: canvas_grep({query:\"TODO\"}) or canvas_grep({query:\"/TODO:\\s+/i\"}).',
    '2) canvas_read_lines uses 1-based inclusive line numbers. Example: canvas_read_lines({start_line:4,end_line:12}).',
    '3) canvas_read_all reads the full canvas. Example: canvas_read_all({}).',
    '4) canvas_apply_patch accepts ONLY unified diff with @@ hunks; no "Begin Patch" blocks. Example:',
    '--- a/canvas',
    '+++ b/canvas',
    '@@ -1,1 +1,2 @@',
    '-old line',
    '+old line',
    '+new line',
    'Line indices are 1-based. Patches must apply cleanly.',
