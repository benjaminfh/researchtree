...
describe('local pg adapter', () => {
  it('builds SQL for set-returning RPCs with positional params', () => {
    const { sql, values, returnType } = buildRpcCall('rt_get_history_v1', baseParams);
    expect(returnType).toBe('set');
    expect(sql).toBe('select * from rt_get_history_v1($1, $2, $3);');
    expect(values).toEqual(['p1', 'main', 50]);
  });
  ...
  it('omits trailing optional params when not provided', () => {
    const { sql, values } = buildRpcCall('rt_get_current_ref_v1', { p_project_id: 'p1' });
    expect(sql).toBe('select rt_get_current_ref_v1($1) as result;');
    expect(values).toEqual(['p1']);
  });

  it('throws when missing a non-trailing param', () => {
    expect(() => buildRpcCall('rt_get_history_v1', { p_project_id: 'p1', p_limit: 2 })).toThrow(
      'Missing parameter p_ref_name for rt_get_history_v1'
    );
  });
  ...
  it('runs and normalizes scalar + set + void responses', async () => {
    const query = vi.fn(async (sql: string) => {
      ...
      if (sql.startsWith('select * from rt_list_refs_v1')) {
        return { rows: [{ name: 'main' }] };
      }
      return { rows: [] };
    });
    ...
    const set = await adapter.rpc('rt_list_refs_v1', { p_project_id: 'p1' });
    const voidResult = await adapter.rpc('rt_set_current_ref_v1', {
      p_project_id: 'p1',
      p_ref_name: 'main',
    ...
    expect(set).toEqual({ data: [{ name: 'main' }], error: null });
  ...
  it('normalizes query errors', async () => {
    ...
    const adapter = createLocalPgAdapter({ query });
    const result = await adapter.rpc('rt_list_refs_v1', { p_project_id: 'p1' });
