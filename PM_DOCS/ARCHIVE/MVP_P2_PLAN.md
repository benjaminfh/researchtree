...
1. **Context builder module (`src/server/context.ts`)**
   ...
   - Use provider/model capability metadata to determine a safe token budget (initially static 8k; post-MVP fetch from `models.list` / Gemini metadata and apply ~0.5 multiplier).
...
6. **Ops**:
   ...
   - Consider adding ESLint rule or simple script to prevent `.js` suffix imports in TS files to avoid regression.

Once these handoff tasks are triaged, Phase 2 can be declared code-complete and we can shift focus to Phase 3 (branch UI + merge semantics).
