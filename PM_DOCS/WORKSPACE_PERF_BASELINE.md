...
4. Capture `[perf-debug] burst` console lines (includes distinct `chatNodeRowRenders` and `nodeBubbleRenders`).

Notes:
- `perfFixtureNodes` supports `1000`, `5000`, `10000` and is only honored when `NEXT_PUBLIC_PERF_DEBUG=1`.
...
- **Render pressure**: `WorkspaceClient` render count per burst should remain within **2x** the keystroke count; inspect `chatNodeRowRenders` and `nodeBubbleRenders` separately for row vs bubble pressure.

## Baseline snapshot (pre-refactor)
| Scenario | Keystrokes | Avg keypress->paint | p95 keypress->paint | WorkspaceClient renders | ChatNodeRow renders | NodeBubble renders | Max dropped frames |
|---|---:|---:|---:|---:|---:|---:|---:|
| 1k nodes / 10s burst | TBD | TBD | TBD | TBD | TBD | TBD | TBD |
| 5k nodes / 10s burst | TBD | TBD | TBD | TBD | TBD | TBD | TBD |
| 10k nodes / 10s burst | TBD | TBD | TBD | TBD | TBD | TBD | TBD |
