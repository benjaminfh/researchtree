-- Fix pgcrypto digest() usage: digest() expects bytea, so hash text via convert_to(...,'utf8')
...
begin
  ...
  v_hash := encode(digest(convert_to(coalesce(p_content, ''), 'utf8'), 'sha256'), 'hex');
...
begin
  ...
  v_content_hash := encode(digest(convert_to(coalesce(p_content, ''), 'utf8'), 'sha256'), 'hex');
