-- RPC: append a provenance node to a ref, optionally snapshotting the caller's current canvas draft
-- into an immutable artefact row on the same commit (no extra "canvas-save" nodes).
