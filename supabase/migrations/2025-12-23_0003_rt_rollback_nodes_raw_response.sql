-- Rollback: remove raw_response support and restore old rt_append_node_to_ref_v1 signature.
-- Context (2025-12-23):
-- A prod deploy picked up supabase/migrations/2025-12-23_0001_rt_nodes_raw_response.sql
-- before the app code that passes p_raw_response, creating an overloaded function ambiguity and
-- breaking writes. This rollback removes the new column + 11-arg function so prod returns to the
-- pre-raw-response schema. Yes, this drops any raw_response data written since that migration.
-- (This was my fuck up; documenting here so we don't repeat it.)

drop function if exists public.rt_append_node_to_ref_v1(
  uuid, text, text, text, jsonb, uuid, text, boolean, text, integer, jsonb
);
...
revoke all on function public.rt_append_node_to_ref_v1(uuid, text, text, text, jsonb, uuid, text, boolean, text, integer) from public;
grant execute on function public.rt_append_node_to_ref_v1(uuid, text, text, text, jsonb, uuid, text, boolean, text, integer) to authenticated;
