-- Add client_request_id for ID-based optimistic reconciliation.
...
begin
  ...
  if p_client_request_id is not null then
    v_content_json := jsonb_set(v_content_json, '{clientRequestId}', to_jsonb(p_client_request_id), true);
  end if;
...
revoke all on function public.rt_append_node_to_ref_v2(uuid, uuid, text, text, jsonb, uuid, text, boolean, text, integer, jsonb, text) from public;
grant execute on function public.rt_append_node_to_ref_v2(uuid, uuid, text, text, jsonb, uuid, text, boolean, text, integer, jsonb, text) to authenticated;
